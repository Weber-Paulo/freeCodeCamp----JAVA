...
Para isso, definiremos um número mínimo mine um número máximo max.

Aqui está a fórmula que usaremos. Leve um momento para ler e entender o que esse código está fazendo:

Math.floor(Math.random() * (max - min + 1)) + min
Crie uma função chamada randomRangeque recebe um intervalo de myMine myMaxe retorne um número inteiro aleatório que é maior ou igual a myMin, e é menor ou igual a myMax.*/
...
function checkEqual(a, b) {
  ...
  /*Usar operadores de condições múltiplas (ternários)
No desafio anterior, você usou um único operador condicional. Você também pode encadear os dois juntos para verificar por condições múltiplas.

A seguinte função usa as instruções if, else ife elsepara verificar múltiplas condições:
...
É considerada a melhor prática para formatar operadores de múltiplas condições, já que cada condição está em uma linha separada, como mostrado acima. Usar operadores de condições múltiplas sem a indentação adequada pode dificultar a leitura do seu código. Por exemplo:

function findGreaterOrEqual(a, b) {
  return (a === b) ? "a and b are equal" : (a > b) ? "a is greater" : "b is greater";
}
Na função checkSign, use operadores de condições múltiplas - seguindo o formato recomendado usado em findGreaterOrEqual- para verificar se um número é positivo, negativo ou zero. A função deve retornar positive, negativeou zero.*/
...
Em um desafio anterior , você aprendeu como usar recursão para substituir laços for. Agora, vamos analisar uma função mais complexa, a qual retorna um array de inteiros consecutivos começando com 1até o número passado para a função.

Como mencionado no desafio anterior, haverá um caso de base . O caso de base diz a função recursiva quando ela não precisa mais chamar a si. É um caso simples onde o valor de retorno já é conhecido. Também haverá uma chamada recursiva , a qual executa a função original com argumentos diferentes. Se a função for escrita corretamente, eventualmente o caso de base será alcançado.

Por exemplo, vamos dizer que você quer escrever uma função recursiva que retorna um array contendo os números de 1até n. Essa função deve aceitar um argumento, n, representando o número final. Então, ela vai precisar chamar a si mesma com valores progressivamente menores, começando em naté chegar a 1. Você poderia escrever a função da seguinte forma:
...
Inicialmente, isso parece contraintuitivo já que o valor de n diminui , mas os valores no array final estão em ordem crescente . Isso acontece porque a adição no array (push) acontece por último, após uma chamada recursiva ter retornado. No ponto onde né adicionado ao array, countup(n - 1)já foi avaliado e avaliado [1, 2, ..., n -1].

Definimos uma função chamada countdowncom um parâmetro ( n). A função deve usar recursão para retornar um array contendo inteiros naté 1com base no parâmetro n. Se a função é chamada com um número menor que 1, a função deve retornar um array vazio. Por exemplo, chamar essa função com n = 5deve retornar o array [5, 4, 3, 2, 1]. Sua função precisa usar recursão para chamar a si mesma e não depender de nenhum tipo de laço.*/
...
Continuando o desafio anterior, fornecemos a você outra oportunidade para criar uma função recursiva para resolver um problema.

Definimos uma função chamada rangeOfNumberscom dois parâmetros. A função deve retornar um array de inteiros a qual começa com um número representado pelo startNumparâmetro e termina com um número representado pelo parâmetro endNum. O número inicial sempre será menor ou igual ao número final. Sua função precisa usar recursão para chamar a si mesma e não deve depender de nenhum tipo de laço. Também deve funcionar para casos onde startNume endNumtiver o mesmo valor.*/
